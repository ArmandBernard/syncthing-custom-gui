...
export interface DbEndpoints {
  ...
  'GET /db/browse': { query: { folder: FolderID; prefix?: string; levels?: number }; response: BrowseEntry[] }
  ...
  'POST /db/ignores': { query: { folder: FolderID }; body: DbIgnoresRequest; response: DbIgnoresResponse }
  'GET /db/localchanged': { query: { folder: FolderID; page?: number; perpage?: number }; response: DbLocalChangedResponse }
  'GET /db/need': { query: { folder: FolderID; page?: number; perpage?: number }; response: DbNeedResponse }
  ...
  'GET /db/remoteneed': { query: { folder: FolderID; device: DeviceID; page?: number; perpage?: number }; response: DbRemoteNeedResponse }
  'POST /db/revert': { query: { folder: FolderID }; response: void }
  /** Array `sub` values are sent as repeated query keys (sub=a&sub=b). */
  'POST /db/scan': { query?: { folder?: FolderID; sub?: string | string[]; next?: number }; response: void }
}
